...
/*
async function habis_vote(conn, x, data) {
  let tok1 = "";
  let tok2 = "";
  let membernya = [];
  ...
  for (let i = 0; i < x.player.length; i++) {
    if (x.player[i].isdead != true) {
      if (x.player[i].role === "werewolf") {
        if (x.player[i].isdead != true) {
          var textnya = `Silahkan pilih player yang ingin kamu bunuh pada malam hari ini\n*LIST PLAYER*:\n${tok2}`;
          let row = [];
          for (let k = 0; k < x.player.length; k++) {
            row.push({
              title: `Kill Player ${x.player[k].number}`,
              rowId: `.wwpc kill ${x.player[k].number}`,
              description: `Untuk membunuh player ${x.player[k].number}`,
            });
          }
          const sections = [{ title: "Werewolf Game", rows: row }];
          const listMessage = {
            text: textnya,
            footer: `Player Hidup: ${playerHidup(x)} Player Mati: ${playerMati(
              x
            )}`,
            title: "Werewolf Game",
          ...
          await conn.sendMessage(x.player[i].id, listMessage);
        }
      } else if (x.player[i].role === "warga") {
        if (x.player[i].isdead != true) {
          let textnya = `Kamu adalah Villager (Warga/Orang Desa) ${emoji_role(
            "warga"
          )}, Silakan masuk kerumah masing masing.\nList Player${tok1}`;
        ...
        if (x.player[i].isdead != true) {
          let textnya = `Kamu memiliki skill spesial, bantu para warga untuk mengunkap sang *Werewolf*, pilihlah player yang ingin kamu buka identitasnya\n*LIST PLAYER*:\n${tok1}`;
          let row = [];
          for (let k = 0; k < x.player.length; k++) {
            row.push({
              title: `Cek Player ${x.player[k].number}`,
              rowId: `.wwpc dreamy ${x.player[k].number}`,
              description: `Untuk melihat identitas player ${x.player[k].number}`,
            });
          }
          const sections = [{ title: "Werewolf Game", rows: row }];
          const listMessage = {
        ...
        if (x.player[i].isdead != true) {
          let textnya = `Sebagai seorang *Guardian Angel* kamu bisa melindungi player saat malam tiba, silahkan pilih salah 1 player yang ingin kamu lindungi\n*LIST PLAYER*:\n${tok1}`;
          let row = [];
          for (let k = 0; k < x.player.length; k++) {
            row.push({
              title: `Lindungi Player ${x.player[k].number}`,
              rowId: `.wwpc deff ${x.player[k].number}`,
              description: `Untuk melindungi player ${x.player[k].number}`,
            });
          }
          const sections = [{ title: "Werewolf Game", rows: row }];
          const listMessage = {
        ...
        if (x.player[i].isdead != true) {
          let textnya = `Kamu adalah teman *Werewolf* yang memiliki kekuatan spesial, bantu *Werewolf* untuk membuka identitas player, silakan pilih 1 orang yang ingin kamu buka identitasnya\n*LIST PLAYER*:\n${tok2}`;
          let row = [];
          for (let k = 0; k < x.player.length; k++) {
            row.push({
              title: `Cek Player ${x.player[k].number}`,
              rowId: `.wwpc sorcerer ${x.player[k].number}`,
              description: `Untuk melihat identitas player ${x.player[k].number}`,
            });
          }
          const sections = [{ title: "Werewolf Game", rows: row }];
...
const pagii = (data) => {
  if (data.dead.length < 1) {
    ...
    for (let i = 0; i < data.dead.length; i++) {
      var b = data.player.findIndex((x) => x.number === data.dead[i]);
...
async function skill(conn, x, data) {
  ...
  if (getWinner(x.room, data).status != null || x.win != null) {
    ...
    for (let i = 0; i < x.player.length; i++) {
      if (x.player[i].role === "werewolf") {
        if (x.player[i].isdead != true) {
          var textnya = `Silahkan pilih salah satu orang yang ingin kamu makan pada malam hari ini\n*LIST PLAYER*:\n${tok2}\n\nKetik *.wwpc kill nomor* untuk membunuh player`;
          /*
          let row = [];
          for (let k = 0; k < x.player.length; k++) {
            row.push({
        ...
        if (x.player[i].isdead != true) {
          let textnya = `Baiklah, siapa yang ingin kamu lihat peran nya kali ini.\n*LIST PLAYER*:${tok1}\n\nKetik *.wwpc dreamy nomor* untuk melihat role player`;
          /*
          let row = [];
          for (let k = 0; k < x.player.length; k++) {
            row.push({
        ...
        if (x.player[i].isdead != true) {
          let textnya = `Kamu adalah seorang*Guardian*, lindungi para warga, silahkan pilih salah 1 player yang ingin kamu lindungi\n*LIST PLAYER*:${tok1}\n\nKetik *.wwpc deff nomor* untuk melindungi player`;
          /*
          let row = [];
          for (let k = 0; k < x.player.length; k++) {
            row.push({
        ...
        if (x.player[i].isdead != true) {
          let textnya = `Baiklah, lihat apa yang bisa kamu buat, silakan pilih 1 orang yang ingin kamu buka identitasnya\n*LIST PLAYER*:${tok2}\n\nKetik *.wwpc sorcerer nomor* untuk melihat role player`;
          /*
          let row = [];
          for (let k = 0; k < x.player.length; k++) {
            row.push({
...
async function win(x, t, conn, data) {
  ...
  if (getWinner(x.room, data).status === false || x.iswin === false) {
    let textnya = `*W E R E W O L F - W I N*\n\nTEAM WEREWOLF\n\n`;
    let ment = [];
    for (let i = 0; i < x.player.length; i++) {
    ...
    let textnya = `*T E A M - W A R G A - W I N*\n\nTEAM WARGA\n\n`;
    let ment = [];
    for (let i = 0; i < x.player.length; i++) {
      if (
