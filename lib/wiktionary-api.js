...
const verify_word = async (word) => {
  try {
    const response = await axios.get("https://id.wiktionary.org/w/api.php", {
      params: {
        "action": "query",
        "list": "search",
        "format": "json",
...
const get_wiki_entry = async (word) => {
  try {
    const response = await axios.get("https://id.wiktionary.org/w/index.php", {
      params: {
        "title": word,
        "printable": "yes"
      }
