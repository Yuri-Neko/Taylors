...
function createTambangs() {
  let tambangs = [
    ...
    {
      area: "Tambang Berlian",
      txt: "berlian",
      reward: { exp: 100, crystal: 50 },
    },
    {
      area: "Tambang Batu Permata",
      txt: "batu",
      reward: { exp: 120, crystal: 60 },
    },
    {
      area: "Tambang Uranium",
      txt: "uranium",
      reward: { exp: 150, crystal: 80 },
    },
    // Tambahkan area tambang lainnya sesuai keinginan
    {
      area: "Tambang Emas Hitam",
      txt: "emas_hitam",
      reward: { exp: 180, crystal: 90 },
    },
    {
      area: "Tambang Kristal",
      txt: "kristal",
...
async function handler(m, { conn, text }) {
  conn.tambang = conn.tambang ? conn.tambang : {};
  ...
  if (m.sender in conn.tambang) {
    ...
    if (text === 'start') {
      ...
      let tambangIndex = Math.floor(Math.random() * tambangs.length);
      let tambang = tambangs[tambangIndex];
      ...
      let { area, txt, reward } = tambangs[tambangIndex];
      let currentArea = 0;
      let hasilTambang = 0;
      let totalReward = 0;
